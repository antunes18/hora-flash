...
# Configuration
AVAILABLE_SLOTS = [f"{h:02d}:00" for h in range(8, 12)] + [
    f"{h:02d}:00" for h in range(14, 18)
]
appointments = {}
lock = threading.Lock()
...
def book_slot():
    ...
    threading.Thread(
        target=whatsapp_service.send_message, args=(phone, message), daemon=True
    ).start()

    return jsonify(
        {
            "message": f"Agendamento confirmado para {time_slot}. A profissional será notificada para confirmar."
        }
    )
