...
class SchedulingService:
    ...
    def create_scheduling(self, dto: scheduling_dto):
        existing = self.scheduling_repo.fing_scheduling_by_date_and_hour(
            dto.date, dto.hour
        )
    ...
    def update_scheduling(self, scheduling_id: int, scheduling: Scheduling):
        scheduling = self.scheduling_repo.find_one_scheduling(scheduling_id)
        if scheduling:
            return self.scheduling_repo.update_scheduling(scheduling_id, scheduling)
        else:
            raise scheduling_exceptions.NotFound()
